...
├── experiments/                       # Probes running the framework against its own claims
│   └── coupling_optimum.py           # 1/φ² placeholder vs the MSF-derived optimum
...
### `experiments/`
Probes that run the framework against its own claims — the mechanism for moving an item out of the falsification log's `Never tested` section. Each prints its own numbers, reruns from one command, and has its findings pinned in `tests/`. `coupling_optimum.py` compares the `1/φ²` placeholder against the MSF-derived optimum by giving each measure its own best coupling scale on the same networks; it found that the Gaussian argmax is `1/φ²` for **every** topology (provably — absent edges contribute a constant to the squared deviation regardless of scale), that severing the one edge holding a barbell together moves `f(C)` by 0.02 while physics reports `FRAGMENTED_STRUCTURALLY`, and that the two measures correlate at **r = +0.055** over 200 random connected graphs. See F-9 and F-10. **When adding a probe here, report what it means for the shipped code too** — these defects are latent, not active, because every audit bridge builds `n = 2`, and saying so is part of the finding.
